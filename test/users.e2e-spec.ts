...
import { getDragonBallZIdFromUrl, idUserNotFound, mockDragonBallZData, userData, userData2 } from './mocks/users.mock';
...
describe('UsersController (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        axiosService = jest.spyOn(axios, 'get').mockImplementation((url: string) => {
            if (url.includes('dragonball-api.com')) {
                ...
                if (id !== null) {
                    const data = mockDragonBallZData.find((dragonBall) => dragonBall.id === id);
    ...
    describe('GET /users/:id', () => {
        it('It should respond 200 code when getting a user successfully', async () => {
            ...
            expect(body.dragonBallZCharacters![0].description).toBe('The strongest being in the universe');
            expect(body.dragonBallZCharacters![0]).toHaveProperty('image');
            expect(body.dragonBallZCharacters![0].image).toBe('https://example.com/vegeta.jpg');
